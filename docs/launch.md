...
```text
In this run, Codex produces a scoped diff, Claude reviews it, and mmux only
applies the patch after the deterministic tester gate passes. The interesting
part is not that two models can talk; it is that they do not own global control.
```
...
- Demo recording shows a real state transition, not only a README scroll.
