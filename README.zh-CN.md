...
默认情况下，worker 只记录 heartbeat 和 role lease，不会改代码。启用 `--execute-agents` 后，持有 `driver` 的 worker 会领取 pending task、获取 resource lock、创建独立 git worktree，并在 worktree 中非交互运行 Codex 或 Claude Code。
