...
- `mmux run --minutes N`：启动一个有时间上限的 tmux 工作区；如果 open queue 为空且时间足够，会先反思新完成任务的 `act_summary`，再退回确定性 frontier / 保守默认任务补给；运行期间定期 checkpoint，到点自动停止并打印任务汇总。
...
- 常驻 agent 输出 `MMUX_DONE task=#N` 后，mmux 会把该 agent 的 resident diff 冻结到普通 task worktree，重置 resident worktree，并把任务推进到 Check 的 review 步（兼容展示名是 `awaiting_review`）；review note 和最终 tester gate 继续决定能否进入主工作区。
...
- `mmux proposed`：只查看等待人工审核的 reflection 草案。
- `mmux task requeue #N`：把 blocked / failed / rejected / no_change / proposed 任务重新放回 `pending`（人工批准 reflection 草案就用它）。
- `mmux reflect`：让 LLM 反思器读最近完成任务的 `act_summary`，给出最多 5 条后续任务建议。引用具体证据（task `#N` 或真实文件路径）的建议会被自动 promote 到 `pending`，含糊的会留在 `proposed` 等人审。这是 Act→Plan 反馈回边；限时运行会在队列空时自动调用这条边。
- 触碰 `src/mmux/` 的 reflection 任务会按自修改任务处理：更小的 diff 上限、额外 `.mmux/self-mutation` 锁，并且必须显式 reviewer approve 后才能进入 tester。
- `mmux roles` / `mmux lease`：查看和管理角色租约。
- `mmux locks` / `mmux lock`：查看和管理资源锁。
- `mmux start --execute-agents`：允许持有 `driver` 的 worker 执行任务。

默认情况下，worker 只记录 heartbeat 和 role lease，不会改代码。推荐用 `mmux run --minutes N --execute-agents` 开启受控的限时自主执行窗口；需要手动观察和接管时，也可以用 `mmux start --execute-agents`。启用执行后，持有 `driver` 的 worker 会领取 pending task、获取 resource lock、创建独立 git worktree，并在 worktree 中先跑一次结构化 plan 步骤：Codex 或 Claude Code 输出一个 JSON `{read, plan, risks}` 契约，末尾以 `MMUX_PLAN PROCEED` 或 `MMUX_PLAN ABORT` 结束。`PROCEED` 的 plan 会先过一道确定性门(`read` 必须非空且引用项目内真实路径,`plan` 必须非空)——在任何模型 reviewer 之前先拦住「没读够就瞎猜」的计划;通过后再过一次 plan reviewer（复用 `MMUX_REVIEW APPROVE` / `REQUEST_CHANGES` 协议）。未过确定性门的契约按 request-changes 处理。approve 通过后 plan 会作为上下文喂给随后的 diff 步骤；第一次 `REQUEST_CHANGES` 会把任务退回 pending 等下一个 driver 重新 plan，第二次拒绝直接进 `blocked`。`ABORT` 让任务直接进入 `no_change`，不再消耗 diff 阶段。

driver 产出的 diff 不会直接进入主工作区。通过路径策略检查后，任务进入 Check 的 review 步；peer `reviewer` 可以结构化 approve 或 request changes，随后通过或 bypass 的 diff 才进入 Check 的 test 步。持有 `tester` 的 worker 会运行确定性检查，通过后才把 patch 应用回主工作区。Patch 应用之后，tester 会调用一次 summarizer，把这轮任务压成 3-5 行 `act_summary` 存进 task payload（改了什么、用什么验证的、有没有意外、下一次要小心什么）。summarizer 是 best-effort：失败只记录，不会阻塞任务进入 `completed`；独立的 `summarizer` role 也会补齐历史 completed task 里缺失的 `act_summary`。限时运行会把新的 act_summary 反思成 `pending` / `proposed` 后续任务。
...
这只是观察模式。它会初始化 `.mmux/`、生成项目画像、在任务队列为空时按 reflection -> frontier -> 保守默认任务的顺序补给、启动 tmux、写 checkpoint，并在到点后自动停止。要允许 Codex 和 Claude Code 在确定性 gate 内实际改代码：
...
stage=plan
  -> stage=do
  -> stage=check check_step=review
  -> stage=check check_step=test
  -> stage=act outcome=completed
```

失败或非接受路径：

```text
outcome=failed / rejected / no_change / blocked
```

- `status` 仍会派生出 `pending` / `awaiting_review` / `awaiting_test` 等旧名字，方便 CLI 阅读；事实源是 `stage` / `outcome` / `in_progress` / `check_step`。
- `driver`：先做 Plan，再在隔离 worktree 中执行模型，产生 diff。
...
- 任务状态以 `stage`、`outcome`、`in_progress`、`check_step` 存储，旧 status 名称只用于可读展示。
...
- 停止运行会清理进行中的 Plan / Do / Check claim，并恢复到可继续阶段。
- 任务进入终态后会把 diff 归档到 `.mmux/archive/` 并删除其 worktree；停止运行时会清理不再属于待 review/待 test 工作的 worktree。设置 `MMUX_KEEP_WORKTREES=1` 可保留 worktree 用于调试。
- 队列为空时，`scout` 角色会让模型基于项目画像和文件树发现 research 型任务；其提案走与 reflection 相同的准入门（证据具体则自动晋升 pending，含糊则停在 `proposed`），模型不可用或没产出时回退到确定性 frontier candidate。
- 队列自动补给会优先使用 Act→Plan reflection，其次使用 scout/frontier candidate，最后才使用通用默认任务。
