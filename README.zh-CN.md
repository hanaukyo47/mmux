...
- `mmux reflect`：让 LLM 反思器读最近完成任务的 `act_summary`，给出最多 5 条后续任务建议。引用具体证据（task `#N` 或真实文件路径）的建议会被自动 promote 到 `pending`，含糊的会留在 `proposed` 等人审。这是 Act→Plan 反馈回边。
...
driver 产出的 diff 不会直接进入主工作区。通过路径策略检查后，任务进入 `awaiting_review`；peer `reviewer` 可以结构化 approve 或 request changes，随后通过或 bypass 的 diff 才进入 `awaiting_test`。持有 `tester` 的 worker 会运行确定性检查，通过后才把 patch 应用回主工作区。Patch 应用之后，tester 会调用一次 summarizer，把这轮任务压成 3-5 行 `act_summary` 存进 task payload（改了什么、用什么验证的、有没有意外、下一次要小心什么）。summarizer 是 best-effort：失败只记录，不会阻塞任务进入 `completed`。把这些 act_summary 转回新任务的反思链路目前还没接上。
